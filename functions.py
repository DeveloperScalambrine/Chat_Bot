...
def atualizar_envio_fotos(user_id: str, consecutivo: bool) -> str:
    ...
    if user:
        if consecutivo:
            new_count = user['fotos_enviadas_para_campanha_de_engajamento'] + 1
            update_user(user_id, 'fotos_enviadas_para_campanha_de_engajamento', new_count)
            return f"{user['nome']} enviou mais uma foto. Agora são {new_count} fotos enviadas."
        else:
            update_user(user_id, 'fotos_enviadas_para_campanha_de_engajamento', 0)
